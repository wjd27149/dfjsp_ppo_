...
for i in range(len(tightness)):
    for j in range(len(length_list)):
        for k in range(len(add_job)):
                # if (i, j, k)  in [(2, 0, 0), (0, 0, 1)]:
                    print(f'm[j], wc[j], length_list[j], tightness[i], add_job[k]: {m[j]}, {wc[j]}, {length_list[j]}, {tightness[i]}, {add_job[k]}')
                    span = 1000
                    tard = []
                    routing_brain = SequencingBrain(span = span , total_episode= total_episode, input_size = 25, output_size = 4)
                    ...
                    save_path = (os.path.join(os.path.dirname(sys.path[0]), 'photo_record','SA'))
