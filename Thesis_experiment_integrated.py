...
class shopfloor:
    def __init__(self, env, span, m_no, wc_no, m_per_wc, tightness,  add_job,**kwargs):
        ...
        if 'address_SA'in kwargs and kwargs['address_SA'] :
            self.sequencing_brain = validation_S.DRL_sequencing(self.env, self.m_list, self.job_creator, \
            address_seed= kwargs['address_SA'], validated=1)
        # specify the architecture of DRL
        if 'address_RA'in kwargs and kwargs['address_RA'] :
            self.routing_brain = validation_R.DRL_routing(self.env, self.job_creator, self.wc_list, address_seed= kwargs['address_RA'], validated=1)
...
# list of experiments
benchmark_1 = [['FIFO','EA'],['FIFO','UT'],['FIFO','SQ'],['FIFO','CT']]
benchmark_2 = [['SPT','EA'],['SPT','UT'],['SPT','SQ'],['SPT','CT']]
benchmark_3 = [['EDD','EA'],['EDD','UT'],['EDD','SQ'],['EDD','CT']]
benchmark_4 = [['CR','EA'],['CR','UT'],['CR','SQ'],['CR','CT']]
    ...
    for j in range(len(length_list)):
        for k in range(len(add_job)):
            for run in range(iteration):
                ...
                for idx,rule in enumerate(benchmark):
                    ...
                    if True:
                        ...
                        spf = shopfloor(env, span, m[j], wc[j], length_list[j], tightness[i], add_job[k], sequencing_rule = rule[0], routing_rule = rule[1])
                        spf.simulation()
                ...
                if True:
                    ...
                    env = simpy.Environment()
                    path_RA = (os.path.join(sys.path[0], 'ddqn_models','RA', f"{m[j]}_{wc[j]}_{tightness[i]}_{add_job[k]}" + ".pt"))
                    path_SA = (os.path.join(sys.path[0], 'ddqn_models','SA', f"{m[j]}_{wc[j]}_{tightness[i]}_{add_job[k]}" + ".pt"))
                    print(path_SA, path_RA)
                    spf = shopfloor(env, span, m[j], wc[j], length_list[j], tightness[i], add_job[k], address_SA = path_SA, address_RA = path_RA)
                    spf.simulation()
                    output_time, cumulative_tard, tard_mean, tard_max, tard_rate = spf.job_creator.tardiness_output()
                    sum_record[run].append(cumulative_tard[-1])
                    max_record[run].append(tard_max)
            ...
            if export_result:
                ...
                parent_dir = os.path.join(sys.path[0], 'Thesis_result_figure','RAW_SA_RA_experiment')
