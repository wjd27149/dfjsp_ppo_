...
class shopfloor:
    def __init__(self, env, span, m_no, wc_no, m_per_wc, tightness,  add_job, **kwargs):
        ...
        if 'address'in kwargs and kwargs['address'] :
            ...
            address_seed= kwargs['address'], validated=1)
...
production_record = {}
# list of experiments
#benchmark = ['ATC','AVPRO','COVERT','CR','CRSPT','EDD','LWKR','LWKRSPT','MDD','MOD','MON','MS','SRMWK','SRMWKSPT','SPT','PTWINQ','PTWINQS','WINQ','DPTWINQNPT','FIFO']
#benchmark = ['ATC','AVPRO','COVERT','CR','EDD','LWKR','MDD','MOD','MON','MS','NPT','SPT','WINQ','LWKRSPT','LWKRMOD','PTWINQ','PTWINQS','DPTLWKRS','DPTWINQNPT','FIFO']
...
benchmark = ['FIFO','SPT','EDD','CR','random_sequencing']
    ...
    for j in range(len(length_list)):
        for k in range(len(add_job)):
            for run in range(iteration):
                ...
                for idx,rule in enumerate(benchmark):
                    ...
                    if True:
                        ...
                        spf = shopfloor(env, span, m[j], wc[j], length_list[j], tightness[i], add_job[k], sequencing_rule = rule)
                        spf.simulation()
                ...
                if True:
                    ...
                    env = simpy.Environment()
                    path = (os.path.join(sys.path[0], 'ddqn_models','SA', f"{m[j]}_{wc[j]}_{tightness[i]}_{add_job[k]}" + ".pt"))
                    print(path)
            ...
            if export_result:
                ...
                raw_address = os.path.join(parent_dir , 'raw_SA.xlsx')
                    # 检查文件是否存在
