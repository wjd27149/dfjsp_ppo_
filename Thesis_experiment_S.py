...
class shopfloor:
    def __init__(self, env, span, m_no, wc_no, m_per_wc, **kwargs):
        ...
        if 'MC' in kwargs and kwargs['MC']:
            print("---> Multi channel state mode ON <---")
...
benchmark = ['FIFO','ATC','AVPRO','COVERT']
...
for run in range(iteration):
    ...
    for idx,rule in enumerate(benchmark):
        ...
        spf = shopfloor(env, span, m_no, wc_no, length_list, sequencing_rule = rule, seed = seed)
        spf.simulation()
    ...
    env = simpy.Environment()
    spf = shopfloor(env, span, m_no, wc_no, length_list, MC=True, seed = seed)
    spf.simulation()
    output_time, cumulative_tard, tard_mean, tard_max, tard_rate = spf.job_creator.tardiness_output()
    sum_record[run].append(cumulative_tard[-1])
    max_record[run].append(tard_max)
...
if export_result:
    ...
    address = sys.path[0]+'\\Thesis_result_figure\\RAW_SA_experiment.xlsx'
